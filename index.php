...
if (isSet($_GET['submit'])) {
    if (isSet($_GET['url']) && isSet($_GET['field-keywords'])) {
        echo 'Started Grabbing ;) - Please be patient<br>';
        $total= 0;
        $amazon = new Amazon();
        $amazon->initlizeNew($_GET['url'], $_GET['field-keywords']);
        $result = $amazon->search();
        $total = count($result['products']);
        
        while($result['success'] && !is_null($result['next']))
        {
			//sleep(2);//sleep 2 seconds [Reduce the load little bit]
			$amazon = new Amazon();
            $amazon->initlizeNext($result['next']);
			
            $result = $amazon->search();
			$total += count($result['products']);
        }
        
        echo 'DONE -> Total Added = '.$total;
        exit();
    }
} else {
    $count = Product::count();
